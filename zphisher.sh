...
##   DDOSOP-IND	: 	Automated Phishing Tool
##   Author 	: 	DarkIceGamer,noobyt
##   Version 	: 	1.1.1
##   Github 	: 	soon
...
check_update(){
	...
	relase_url='https://api.github.com/repos/htr-tech/DDOSOP-IND/releases/latest'
	new_version=$(curl -s "${relase_url}" | grep '"tag_name":' | awk -F\" '{print $4}')
	tarball_url="https://github.com/htr-tech/DDOSOP-IND/archive/refs/tags/${new_version}.tar.gz"
	...
	if [[ $new_version != $__version__ ]]; then
		...
		--retry 3 --retry-delay 2 --location --output ".DDOSOP-IND.tar.gz" "${tarball_url}"

		if [[ -e ".DDOSOP-IND.tar.gz" ]]; then
			tar -xf .DDOSOP-IND.tar.gz -C "$BASE_DIR" --strip-components 1 > /dev/null 2>&1
			[ $? -ne 0 ] && { echo -e "\n\n${RED}[${WHITE}!${RED}]${RED} Error occured while extracting."; reset_color; exit 1; }
			rm -f .DDOSOP-IND.tar.gz
			popd > /dev/null 2>&1
			{ sleep 3; clear; banner_small; }
			echo -ne "\n${GREEN}[${WHITE}+${GREEN}] Successfully updated! Run DDOSOP-IND again\n\n"${WHITE}
			{ reset_color ; exit 1; }
		else
